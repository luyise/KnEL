...
let rec checktype_of_tactic (env: tactic_type SMap.t) expected parsed_tac : tactic_type =
    ...
    begin match parsed_tac with
      ...
      | PTacSeq (pt1, pt2) | PTacOr (pt1, pt2) ->
        ...
        then TTac
            ...
            Format.printf "%a - %a - %a\n" pp_parsed_tactic parsed_tac pp_tactic_type (checktype_of_tactic env TTac pt1) pp_tactic_type (checktype_of_tactic env TTac pt2);
  ...
  | TTerm ->
    begin match parsed_tac with
      ...
      | _ -> (Format.printf "wanted term : %a\n" pp_parsed_tactic parsed_tac; raise TacticTypeError)
    end
  | TArrow (t1, t2) ->
    begin match parsed_tac with
      | PTacVar ident ->
...
let defaultTacticsList = [
  ...
  "Split", TTac, Tactic (PTacBase TISplit);
  "ProdRec", TArrow (TIdentL, TTac), Arg ("l", TIdentL, Tactic (PTacApp (PTacBase TIProdRec, PTacVar "l")));
  "Choose", TArrow (TInt, TTac), Arg ("i", TInt, Tactic (PTacApp (PTacBase TIProdRec, PTacVar "i")));
  "SumRec",  TTac, Tactic (PTacBase TISumRec);
...
let rec term_of_ptac env = function
    ...
    | PTacApp (pt1, pt2) -> TApp (term_of_ptac env pt1, term_of_ptac env pt2)
    | _ -> assert false
...
let rec compute_tactic args (env: tactic_ctxt) parsed_tac = match args, parsed_tac with
    ...
    | [], PTacBase TISplit -> BaseTac SplitTac
    | [x], PTacBase TIProdRec -> BaseTac (ProdRecTac (get_list env x))
    | [x], PTacBase TIChoose -> BaseTac (ChooseTac (get_int env x))
    | [], PTacBase TISumRec -> BaseTac SumRecTac
